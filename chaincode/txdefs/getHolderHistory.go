...
type HolderHistoryRecord struct {
...
var GetHolderHistory = tx.Transaction{
	Tag:         "getHolderHistory",
	Label:       "Get Holder History",
	Description: "Get Holder History",
	Method:      "GET",
	...
	Routine: func(stub *stubwrapper.StubWrapper, req map[string]interface{}) ([]byte, errors.ICCError) {
		id, ok := req["holderId"].(string)
		if !ok {
			return nil, errors.WrapError(nil, "Invalid type for parameter 'holderId'")
		}
		...
		if err != nil {
			return nil, errors.WrapError(err, "failed to create key for holder")
		}

		_, err = contractId.Get(stub)
		if err != nil {
			return nil, errors.WrapError(err, "Failed to get holder asset from the ledger")
		}

		historyIterator, err := stub.GetHistoryForKey(contractId.Key())
		if err != nil {
			return nil, errors.WrapError(err, "failed to read holder history from blockchain")
		}
